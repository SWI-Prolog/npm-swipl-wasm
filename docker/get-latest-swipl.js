...
async function main() {
  ...
  if (isHigherVersion(bestElem.name, package.config.swipl.version)) {
    package.config.swipl.version = bestElem.name;
    package.config.swipl.commit = bestElem.commit.sha;
    fs.writeFileSync(path.join(__dirname, '..', 'package.json'), `${JSON.stringify(package, null, 2)}\n`);
  }
}

function isHigherVersion(v1, v2) {
  const [major1, minor1, patch1] = v1.slice(1).split('.').map(e => parseInt(e));
  const [major2, minor2, patch2] = v2.slice(1).split('.').map(e => parseInt(e));
